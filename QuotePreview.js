        ...
        if (centrelong !== "") {
            ...
            for (i = 0; i < stops; i++) {
                marker = new google.maps.Marker({
                    position: new google.maps.LatLng(cruiseLocations[i].Latitude, cruiseLocations[i].Longitude),
                    map: map,
                    title: cruiseLocations[i].Name,
                    label: { text: cruiseLocations[i].Day, color: "white" }
                })
                const infowindow = new google.maps.InfoWindow({ content: cruiseLocations[i].Name })
...
function initAccommodationMap() {
    let accommodationMap = new google.maps.Map(document.getElementById("accommodationMap"),
        {
